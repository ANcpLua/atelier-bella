...
You are integrating components and blocks from **React Bits Pro**, a premium shadcn-compatible component registry with
100+ animated components and 158+ page section blocks for React/Next.js applications.

## Installing this skill via CLI

If the user's project already has a `components.json` with the `@reactbits-starter` registry configured, you can install
...
Components are installed directly into the user's codebase (not as npm packages). They become local source files the
user owns and can modify.
...
3. **A valid license key**. The user must have purchased a React Bits Pro license. Ask the user for their license key if
...
Add the React Bits Pro registries to the `registries` field in `components.json`. If the `registries` key does not
exist, create it.
...
**Do NOT overwrite** existing `components.json` fields (`$schema`, `style`, `tailwind`, `aliases`, etc.). Only add or
merge the `registries` object.
...
- **Blocks** use `named` exports matching PascalCase of slug: `import { Hero1 } from "@/components/blocks/hero-1"`
  - The exported function name is the PascalCase version: `hero-1` → `Hero1`, `pricing-3` → `Pricing3`,
...
Many components use specialized libraries. When you install via the shadcn CLI, dependencies are automatically resolved.
However, be aware of these key dependencies so you can help the user troubleshoot:
...
ALL React Bits Pro components and blocks include `"use client"` at the top. They are client components. If you are using
them in a Server Component page in Next.js App Router, simply import them — Next.js handles the client boundary
automatically. Do NOT try to remove the `"use client"` directive.
...
All blocks and most components support dark mode via Tailwind's `dark:` variant. They use the standard `dark` class
strategy. Ensure your app has a theme provider set up:
...
The user's license tier doesn't include access to the requested content. Starter licenses cannot access `@reactbits-pro`
blocks — a Pro or Ultimate license is required.
...
Install the utility: `npm install clsx tailwind-merge`, then create `lib/utils.ts` with the `cn` function shown in Step
...
7. **Preview before installing** — use `npx shadcn@latest view @reactbits-starter/silk-waves-tw` to inspect a component
   before adding it
8. **Install this skill into projects** — run `npx shadcn@latest add @reactbits-starter/skill` to drop this SKILL.md
   into the project root so agents always have it available locally
